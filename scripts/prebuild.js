...
function changeFile(item) {
  ...
  if (data) {
    if (
      data.indexOf(item.newStr) !== -1 &&
      data.indexOf(item.oldStr) === -1 &&
      !item.noRegExpEscape &&
...
function replaceText(data, item) {
  ...
  if (item.noRegExpEscape) {
    if (repTypeIsString) {
      ...
      item.oldStr.forEach((str, index) => {
        newFile = newFile.replace(
          new RegExp(item.oldStr[index], 'g'),
          item.newStr[index]
        )
      })
      // console.log(newFile)
    ...
    if (repTypeIsString) {
      newFile = data.replace(
        new RegExp(RegExp.escape(item.oldStr), 'g'),
        item.newStr
      )
    } else {
      item.oldStr.forEach((str, index) => {
        newFile = newFile.replace(
          new RegExp(RegExp.escape(item.oldStr[index]), 'g'),
          item.newStr[index]
        )
      })
    }
