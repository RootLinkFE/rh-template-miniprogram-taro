...
class RhTabbar extends React.Component<PropType, StateType> {
  static _tabBar: any = Taro.getCurrentInstance()

  constructor(props: PropType) {
    super(props)
    const tabBarList = RhTabbar._tabBar.app.config.tabBar.list.map(
      fixListConfig
    )
    this.state = {
      activeIdx: tabBarList.find((item) => item.pagePath === `${props.route}`)
        .idx,
      config: {
        ...RhTabbar._tabBar.app.config.tabBar,
        customNode: Taro.getApp().$app.customNode,
      },
      list: tabBarList || [],
      // 自定义节点
      customOrder:
        Math.floor(RhTabbar._tabBar.app.config.tabBar.list.length / 2) - 1,
      customActive: false,
